...
- Built by `build_context_snapshot()` in `src/swarmee_river/harness/context_snapshot.py`.
...
7) **Project map**
- Built alongside preflight in `src/swarmee_river/harness/context_snapshot.py`.
...
- Preflight snapshot content + size: `src/swarmee_river/harness/context_snapshot.py`
...
- **Tool consent defaults**: edit `.swarmee/settings.json` under `safety` (parsed in `src/swarmee_river/settings.py`; enforced by `src/swarmee_river/hooks/tool_consent.py`).
...
- Context snapshot text: `src/swarmee_river/harness/context_snapshot.py`
