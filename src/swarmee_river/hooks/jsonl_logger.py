...
def _secret_values_from_env() -> list[str]:
    ...
    # De-dup, longest first to avoid partial overlaps.
...
def _redact_obj(value: Any, secrets: list[str]) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            key = str(k)
            if any(part in key.upper() for part in ("KEY", "SECRET", "TOKEN", "PASSWORD")):
                out[key] = "<redacted>"
            else:
                out[key] = _redact_obj(v, secrets)
        return out
    if isinstance(value, list):
        return [_redact_obj(v, secrets) for v in value]
    if isinstance(value, tuple):
        return tuple(_redact_obj(v, secrets) for v in value)
    return value


def _extract_usage_payload(event: Any) -> Any:
...
class JSONLLoggerHooks(HookProvider):
    ...
    def after_invocation(self, event: AfterInvocationEvent) -> None:
        ...
        result_text = str(event.result) if event.result is not None else None
        if isinstance(result_text, str) and self.redact:
            result_text = _redact_text(result_text, self._secrets)
        result_repr = _truncate(result_text, self.max_field_chars) if result_text is not None else None
        self._log(
            "after_invocation",
            {
                "invocation_id": inv_id,
    ...
    def before_tool_call(self, event: BeforeToolCallEvent) -> None:
        ...
        if tool_input and self.redact:
            tool_input = _redact_obj(tool_input, self._secrets)
    ...
    def after_tool_call(self, event: AfterToolCallEvent) -> None:
        ...
        result = event.result
        if result and self.redact:
            result = _redact_obj(result, self._secrets)
        result_repr = _truncate(_safe_json(result), self.max_field_chars) if result else None
        self._log(
            "after_tool_call",
            {
                "invocation_id": inv_id,
    ...
    def after_model_call(self, event: AfterModelCallEvent) -> None:
        ...
        if response_payload is not None and self.redact:
            response_payload = _redact_obj(response_payload, self._secrets)
